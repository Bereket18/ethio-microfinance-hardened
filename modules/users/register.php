...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (strlen($username) < 3) {
        $message = "Username must be at least 3 characters."; $message_type = 'danger';
    } elseif (!validate_password($password)) {
        $message = "Password must be at least 6 characters."; $message_type = 'danger';
    } elseif ($email !== '' && !validate_email($email)) {
        $message = "Please enter a valid email address."; $message_type = 'danger';
    } else {
        ...
        if ($exists) {
            $message = "That username is already taken."; $message_type = 'danger';
        } else {
            ...
            if ($result) {
                ...
                $message = "Registration failed. Please try again."; $message_type = 'danger';
            }
        }
    }
}
...
<div class="auth-brand"><h2><i class="bi bi-bank2"></i> EthioMF</h2><p>Create your account and start banking</p></div>
...
<input type="text" name="username" class="form-control" placeholder="Choose a username" minlength="3" required></div>
...
<button type="button" class="password-toggle position-absolute top-50 end-0 translate-middle-y border-0 bg-transparent me-2" style="z-index: 5;"><i class="bi bi-eye text-white-50"></i></button></div>
<small class="text-muted">Password must be at least 6 characters</small></div>
